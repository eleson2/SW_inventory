...
import { cloneSoftware, clonePackage, cloneLpar, getClonePreview } from '$lib/server/clone-utils';
import { z } from 'zod';

// Clone request schema
const cloneRequestSchema = z.object({
	entityType: z.enum(['software', 'package', 'lpar']),
...
export const GET: RequestHandler = async ({ url }) => {
	try {
		const entityType = url.searchParams.get('entityType') as 'software' | 'package' | 'lpar';
