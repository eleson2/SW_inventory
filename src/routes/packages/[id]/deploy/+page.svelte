<script lang="ts">
	...
	const summary = $derived(() => {
		...
		const changes = selectedLpars.reduce((sum, lpar) => sum + (lpar.changesNeeded || 0), 0);
		const newInstalls = selectedLpars.reduce((sum, lpar) => sum + (lpar.newInstalls || 0), 0);
		return { total, changes, newInstalls };
	});

	function getLabel(lpar: LparWithStatus): string {
		return `${lpar.customers.name} - ${lpar.name}`;
	}

	function getSubLabel(lpar: LparWithStatus): string {
		if (lpar.packages) {
			return `Current: ${lpar.packages.name} (${lpar.packages.version})`;
		}
	...
	function getStatusIcon(lpar: LparWithStatus): string {
		...
		if ((lpar.changesNeeded || 0) + (lpar.newInstalls || 0) > 0) return '⚠️';
		return '';
	}

	function getStatusColor(lpar: LparWithStatus): string {
		if (lpar.isCompliant) return 'text-green-600';
		if ((lpar.changesNeeded || 0) + (lpar.newInstalls || 0) > 0) return 'text-amber-600';
		return 'text-muted-foreground';
	}

	function groupBy(lpar: LparWithStatus): string {
		return lpar.customers.name;
	}
	...
	const confirmationDetails = $derived(() => {
		...
		selectedLpars.forEach((lpar) => {
			const status = lpar.isCompliant
				? '✅ Compliant'
				: `⚠️ ${(lpar.changesNeeded || 0) + (lpar.newInstalls || 0)} changes needed`;
			details.push(`• ${lpar.customers.name} - ${lpar.name} (${status})`);
		});
...
<div class="space-y-6 max-w-7xl">
	...
	<div class="flex justify-start">
		<TransactionId prefix="DEPLOY" label="Deployment ID" />
	</div>
	...
	{#if selectedLpars.length > 0}
		<Card class="p-6">
			...
			<div class="space-y-4">
				...
				<!-- Deployment Mode -->
				<div class="space-y-2">
					<div class="text-sm font-medium">Deployment Strategy</div>
		...
		{:else if previewData === null && selectedLpars.length > 0}
			<Card class="p-6 border-amber-500">
				<div class="text-center py-8">
					<p class="font-semibold mb-2 text-amber-600">⚠️ Preview Not Available</p>
					<p class="text-sm text-muted-foreground">
						Unable to load deployment preview. You can still proceed with deployment.
					</p>
				</div>
			</Card>
		...
		<Card class="p-6">
			<div class="flex gap-3">
				<Button
					...
					disabled={isDeploying || selectedLpars.length === 0}
				>
